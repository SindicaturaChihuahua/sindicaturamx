...
if(logeado() && $user->exists){
	if(isset($_REQUEST['id']) && isset($_REQUEST['estilo'])){
		...
		if($user->uid==$_REQUEST['id'] && isset($controlador->single_files) && isset($controlador->single_files[$_REQUEST['estilo']])){
			$tipo=$_REQUEST['estilo'];
			$access_granted=true;
			$folderfinal=$path."obj".$user->uid."/";
			
			if($accion=='checkfile'){
				$preview_file=SIGA.'images/siga_default_file.png';
				$delete_url=$thisurl.'?accion=delete&estilo='.$_REQUEST['estilo'].'&id='.$user->uid;
				if(validaGeneral($user->{$_REQUEST['estilo']},4)){
					$file_generated=URLPAGES.'usuarios/obj'.$user->uid.'/'.$user->{$_REQUEST['estilo']};
					if($controlador->single_files[$_REQUEST['estilo']]['tipoarchivo'] == 'image'){
						$preview_file=URLPAGES.'usuarios/obj'.$user->uid.'/small_'.$user->{$_REQUEST['estilo']};
					}else{
				...
				$response_json=false;
				$confirm_delete_url=$thisurl.'?accion=confirmdelete&estilo='.$_REQUEST['estilo'].'&id='.$user->uid;
				?>
				...
				if(validaGeneral($user->{$_REQUEST['estilo']},4)){
					if($controlador->single_files[$_REQUEST['estilo']]['tipoarchivo']=='image'){
						deleteFiles($user->{$_REQUEST['estilo']},$folderfinal,array('big_','medium_','small_','xsmall_'));
					}else{
						deleteFiles($user->{$_REQUEST['estilo']},$folderfinal,array());	
					}
				}
				$stmt = $db->prepare("UPDATE ".DB_PREFIX.$controlador->databaseprefix." SET ".$_REQUEST['estilo']." = :cover WHERE post_id = :theobjid");
				if($stmt->execute(array(":cover"=>'',":theobjid"=>$user->uid))){
					$message='';
				}
				echo json_encode(
					array(
	...
	if($access_granted && count($errores)==0 && isset($havecover) && count($havecover)>0){
		...
		foreach($havecover as $newfile){
			if(rename($folderpath_tmp.$newfile[1],$folderfinal.$newfile[1])){
				...
				$file_generated=URLPAGES.'usuarios/obj'.$user->uid.'/'.$newfile[1];
				if($controlador->single_files[$newfile[0]]['tipoarchivo']=='image'){
					...
					deleteFiles($user->{$newfile[0]},$folderfinal,array('big_','medium_','small_','xsmall_'));
					$preview_file=URLPAGES.'usuarios/obj'.$user->uid.'/small_'.$newfile[1];
				}else{
				...
				if($stmt->execute(array(":cover"=>$newfile[1],":theobjid"=>$user->uid))){
					$message='';
				}
			}else{
