	...
	public function printModalAllMiembro(){
		...
		if(!empty($this->comisiones)){
			...
			for($i=0; $i < count($this->comisiones); $i++){
				...
				$comisiones .= '<a href="'.URL.'ayuntamiento/comisiones/'.$comisionesnombres[$this->comisiones[$i]["comision_id"]]["slug"].'">-'.$this->comisiones[$i]["comision_puesto"].' de la Comisión de '.$comisionesnombres[$this->comisiones[$i]["comision_id"]]["titulo"].'</a><br>';
			}
		}
		echo '<div class="modal-miembro modal-'.$this->slug.'">
			<div class="wrap">
