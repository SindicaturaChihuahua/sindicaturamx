		...
		if(!empty($this->byYear)){
			foreach ($this->byYear as $y => $obras) {
				...
				echo '<div class="obras a-'.$y.'">
					...
					$categorias = getCategoriasObras();
					$obrasGrouped = $this->groupByCategoria($obras,"seccion");
					foreach ($categorias as $key => $value) {
						if(!empty($obrasGrouped[$key])){
							echo '<div class="obras-seccion os-'.$key.$y.'">
								<div class="obras-seccion-nombre abrir-seccion" data-open="'.$key.$y.'">'.$value.'</div>
								<div class="obras-seccion-contenido osc-'.$key.$y.'">';
									for ($i=0; $i < count($obrasGrouped[$key]); $i++) {
										$obrasGrouped[$key][$i]->printObra();
									}
							echo '</div>
							</div>';
						}
