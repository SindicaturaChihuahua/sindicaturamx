...
class Auditorias {
	...
	public function get($pagina=1, $limit=10, $categoria = false, $tipo=false){
		...
		if($stmt = $this->db->prepare($query)){
			...
			foreach($auditorias as $key => $tmpauditoria){
			 $auditoria = new Auditoria($this->db);
			 $auditoria->initFromData($tmpauditoria);
	...
	public function printAuditorias(){
		if(count($this->auditorias) > 0){
			for($i=0; $i < count($this->auditorias); $i++){
				$this->auditorias[$i]->printAuditoria();
			}
		}
	}
	public function printAuditoriasByCategoria(){
		if(empty($this->categorias)){
			$this->getCategorias();
		}
		foreach($this->categorias as $categoria){
			if($categoria["no_empty"]){
				$auditorias = new Auditorias($this->db);
				$auditorias->get(1,25,$categoria["categoria_id"]);
				echo '<div class="reset-table comision-sesiones auditorias-tabla">
				    ...
				    <table class="comision-sesiones-tabla">
				        ...
				        <tbody>';
						for($i=0; $i < count($auditorias->auditorias); $i++) {
							$auditorias->auditorias[$i]->printAuditoria();
						}
						echo '
				        </tbody>
				    </table>
