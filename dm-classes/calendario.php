...
class Eventos {
	...
	'dia' => array("Domingo","Lunes","Martes","Miércoles","Jueves","Viernes","Sábado"),
		...
		$this->comisiones = $this->getComisiones();
	}
	...
	public function printEventosHeader(){
		...
		echo '<h2 class="tcenter"><a href="'.URL.'ayuntamiento/calendario-de-regidores/'.$this->prev.'#vC"><i class="fas fa-chevron-left"></i></a> '.$this->nombre_mes.' '.$this->year.' <a href="'.URL.'ayuntamiento/calendario-de-regidores/'.$this->next.'#vC"><i class="fas fa-chevron-right"></i></a></h2>';
	...
	public function printComisiones(){
		...
		if(isset($_GET["m"]) && isset($_GET["a"])){
			$addurl = '/?m='.$_GET["m"].'&a='.$_GET["a"].'#vC';
		}
		echo '<ul>';
		echo '<li><a href="'.URL.'ayuntamiento/calendario-de-regidores'.$addurl.'"'.$activeI.'>Mostrar todos</a></li>';
		foreach($this->comisiones as $comision){
			if($comision["no_empty"]){
				$active = '';
				if(isset($_GET["ver"]) && !empty($_GET["ver"]) && $_GET["ver"] == $comision["slug"]){
					$active = ' class="active"';
				}
				echo '<li><a href="'.URL.'ayuntamiento/calendario-de-regidores'.$addurl.'&ver='.$comision["categoria_slug"].'"'.$active.'>'.$comision["titulo"].'</a></li>';
			}
		}
		echo '</ul>';
	}
	...
	function noEmptyComision($comision){
		$stmt = $this->db->prepare("SELECT COUNT(t1.post_id) FROM ".DB_PREFIX.$this->databaseprefix." AS t1 INNER JOIN ".DB_PREFIX."s_acomisiones AS t3 ON t1.post_id = t3.post_id WHERE t1.categoria_id = ".$categoria." AND t1.status = 'publicado' AND visibilidad = 'publico' ORDER BY t1.modificado DESC");
		$stmt->execute();
		$total_obj = $stmt->fetch(PDO::FETCH_COLUMN);
	    if($total_obj > 0){
			return true;
