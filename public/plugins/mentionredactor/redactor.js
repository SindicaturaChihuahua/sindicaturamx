...
(function($)
{
	...
	var uuid = 0;
	var offsetTopFolklore = 76; //folklore
		...
		build: function()
		{
			return {
				...
				setEvents: function()
				{
					...
					// keypress & input Folklore
					this.$editor.on('keypress.redactor', function(e){$(document).trigger('redactor.keypress', e.originalEvent);});
					// if (navigator.userAgent.indexOf("MSIE 8") > -1) {
					// 	this.$editor.on('propertychange.redactor', function(e){$(document).trigger('redactor.keyinput', e.originalEvent);});
					// } else {
					// 	this.$editor.on('input.redactor', function(e){$(document).trigger('redactor.keyinput', e.originalEvent);});
					// }
			...
			return {
				...
				show: function(e, key)
				{
					...
					else
					{
						...
						{
							var top = this.$toolbar.innerHeight() + this.opts.toolbarFixedTopOffset + offsetTopFolklore;
							var position = 'fixed';
							if (this.opts.toolbarFixedTarget !== document)
							{
								top = (this.$toolbar.innerHeight() + this.$toolbar.offset().top) + this.opts.toolbarFixedTopOffset + offsetTopFolklore;
								position = 'absolute';
							}
		...
		toolbar: function()
		{
			return {
				...
				observeScroll: function()
				{
					...
					if (this.opts.toolbarFixedTarget === document)
					{
						boxTop = this.$box.offset().top - offsetTopFolklore;
					}
				...
				observeScrollEnable: function(scrollTop, boxTop)
				{
					...
					//Folklore Scroll
					var v = (scrollTop < end) ? 'visible' : 'hidden';
					if(typeof this.$mentionslist !== 'undefined' && this.$mentionslist[0]){
						this.$mentionslist.css({
							position: 'absolute',
							top: (top + 32) + 'px',
						});
						this.$mentionslist.css('visibility', v);
					}else{
						this.$mentionslist = $(this.$box).find('.mentions-autocomplete-list');
					}

					this.toolbar.setDropdownsFixed();
					this.$toolbar.css('visibility', v);
				},
