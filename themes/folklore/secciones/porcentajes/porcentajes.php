<?php
class back_porcentajes extends Seccion {

	function __construct($db)
	{
		$this->db = $db;
		$this->backclass = "back_porcentajes";
		$this->frontclass = "front_porcentajes";
		...
		$this->single_files = array(
			'cover' => array(
				...
				'maxfilesize' => '10',
				'info_btn' => '<span class="fa fa-file-image-o"></span> &nbsp;Gráfica',
				'info' => 'Selecciona la imagen que contenga la gráfica para este rubro.',
				'tipoarchivo' => 'image',
				'extra' => false
			),
		);
	...
	public function call($accion,$gama=false,$delta=false) {
		if($accion=="lista"){
			...
			if($gama=="rubro"){
				return array(
					"file" => "backend/rubro-nuevo-handler",
					"ajax" => true
				);
			}
