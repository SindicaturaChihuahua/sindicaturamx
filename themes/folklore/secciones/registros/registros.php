<?php
class back_registros extends Seccion {
	...
	function __construct($db)
	{
        ...
        $this->version = '1.0';
		$this->tab = 'editor';
		$this->displayName = 'Registros';
		$this->description = 'Slider';
		$this->icon = 'image';
		$this->databaseprefix = 's_registros';
		$this->permiso = 'Editor';
	...
	public function getComisiones(){
		$stmt = $this->db->prepare("SELECT * FROM ".DB_PREFIX."s_acomisiones WHERE status = ? AND visibilidad = ?");
		$stmt->execute(array('publicado','publico'));
