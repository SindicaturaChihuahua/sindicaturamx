<?php
class back_publicaciones extends Seccion {

	function __construct($db)
	{
		$this->db = $db;
		$this->backclass = "back_publicaciones";
		$this->frontclass = "front_publicaciones";
		...
		$this->single_files = array(
			'cover' => array(
				...
				'info' => 'Selecciona la imagen principal del publicacion',
				'tipoarchivo' => 'image',
				'extra' => false
			)
		);
		...
		$this->configuracion_default = array(
			"formatopublicacion" => "publicacion"
		);
		$this->videos = array(
			"" => "-Selecciona uno-",
			"facebook" => "Facebook",
	...
	public function call($accion,$gama=false,$delta=false) {
		if($accion=="lista"){
			...
			if($gama=="publicacion"){
				return array(
					"file" => "backend/publicacion-nuevo-handler",
					"ajax" => true
				);
			}
