...
if(logeado() && $user->exists && $user->hasRole($controlador->permiso)){
	...
	if(isset($_GET['beta']) && $_GET['beta']=='editar' && isset($_GET['delta']) && $obj = $controlador->getPost($_GET['delta'], $_GET['gama'])){
		$cHeaderData['titulo'] = 'Editar publicación';
		$url->addBreadcrumb(array('nombre' => 'Editar publicación', 'link' => 's/'.$controlador->name.'/editar/'.$obj['tipo'].'/'.$obj['post_id']));
		...
		$cHeaderData['titulo'] = 'Nueva publicación';
		$url->addBreadcrumb(array('nombre' => 'Nueva publicación', 'link' => 's/'.$controlador->name.'/nuevo/'.$obj['tipo']));
	}else{
            ...
            <div class="col-sm-9">
                <div class="row">
                    <div class="col-sm-12">
						...
						<div class="form-group">
                            <label class="control-label">Nombre del medio</label>
                            <input type="text" class="form-control" name="tagline" value="<?=cleanOutput($obj['tagline']);?>">
                        </div>
						<div class="form-group">
                            <label class="control-label">Enlace</label>
                            <input type="text" class="form-control" name="url" value="<?=cleanOutput($obj['url']);?>">
                        </div>
                        <!-- <div class="form-group">
