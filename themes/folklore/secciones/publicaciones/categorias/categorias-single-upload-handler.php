...
$response_json=true;
$message="";
$path="../public/cargas/".$controlador->name."/categorias/";
$accion	= isset($_REQUEST['accion']) ? $_REQUEST['accion'] : 'uploadfile';
$thisurl=$url->createUrl($controlador->static_urls['categorias-single-upload-handler']);
...
if(logeado() && $user->exists && $user->hasRole("EditorEnJefe")){
	if(isset($_REQUEST['id']) && isset($_REQUEST['estilo'])){
		...
		if($object && isset($controlador->categorias_single_files) && isset($controlador->categorias_single_files[$_REQUEST['estilo']])){
			$tipo=$_REQUEST['estilo'];
			$access_granted=true;
			$folderfinal=$path."obj".$object['categoria_id']."/";

			if($accion=='checkfile'){
				$preview_file=SIGA.'images/siga_default_file.png';
				$delete_url=$thisurl.'?accion=delete&estilo='.$_REQUEST['estilo'].'&id='.$object['categoria_id'];
				if(validaGeneral($object[$_REQUEST['estilo']],4)){
					$file_generated=URLCARGAS.$controlador->name.'/categorias/obj'.$object['categoria_id'].'/'.$object[$_REQUEST['estilo']];
					if($controlador->categorias_single_files[$_REQUEST['estilo']]['tipoarchivo'] == 'image'){
						$preview_file=URLCARGAS.$controlador->name.'/categorias/obj'.$object['categoria_id'].'/small_'.$object[$_REQUEST['estilo']];
					}else{
				...
				$response_json=false;
				$confirm_delete_url=$thisurl.'?accion=confirmdelete&estilo='.$_REQUEST['estilo'].'&id='.$object['categoria_id'];
				?>
				...
				$stmt = $db->prepare("UPDATE ".DB_PREFIX.$controlador->databaseprefix."_categorias SET ".$_REQUEST['estilo']." = :cover WHERE categoria_id = :theobjid");
				if($stmt->execute(array(":cover"=>'',":theobjid"=>$object['categoria_id']))){
					$message='';
				}
				echo json_encode(
					array(
				...
				if(isset($_FILES)){
					foreach($_FILES as $type => $file){
						if(isset($file) && "file_".$tipo==$type && isset($controlador->categorias_single_files[$tipo]) && strlen($file['tmp_name'])>2){
							$ext = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
							if(in_array($ext,$controlador->categorias_single_files[$tipo]['required']) && ($file['size']/MEGABYTE)<=$controlador->categorias_single_files[$tipo]['maxfilesize']){
	...
	if($access_granted && count($errores)==0 && isset($havecover) && count($havecover)>0){
		...
		foreach($havecover as $newfile){
			if(rename($folderpath_tmp.$newfile[1],$folderfinal.$newfile[1])){
				...
				$file_generated=URLCARGAS.$controlador->name.'/obj'.$object['categoria_id'].'/'.$newfile[1];
				if($controlador->categorias_single_files[$newfile[0]]['tipoarchivo']=='image'){
					...
					$preview_file=URLCARGAS.$controlador->name.'/obj'.$object['categoria_id'].'/small_'.$newfile[1];
				}else{
					deleteFiles($object[$newfile[0]],$folderfinal,array());
					$preview_file=SIGA.'images/siga_default_file_up.png';
				}

				$stmt = $db->prepare("UPDATE ".DB_PREFIX.$controlador->databaseprefix."_categorias SET ".$newfile[0]." = :cover WHERE categoria_id = :theobjid");
				if($stmt->execute(array(":cover"=>$newfile[1],":theobjid"=>$object['categoria_id']))){
					$message='';
				}
			}else{
