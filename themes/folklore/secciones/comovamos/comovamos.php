<?php
class back_comovamos extends Seccion {

	function __construct($db)
	{
		$this->db = $db;
		$this->backclass = "back_comovamos";
		$this->frontclass = "front_comovamos";
        $this->name = "comovamos";
        $this->order = '1';
        $this->version = '1.1.0';
		$this->tab = 'editor';
		...
		$this->single_files = array(
			'cover' => array(
				...
				'info' => 'Selecciona la imagen principal del indicador',
				'tipoarchivo' => 'image',
				'extra' => false
			),
			'thumb' => array(
				'required' => array("jpg","jpge","jpeg","gif","png"),
				'maxfilesize' => '10',
				'info_btn' => '<span class="fa fa-file-image-o"></span> &nbsp;Imagen interior',
				'info' => 'Selecciona la imagen interior del indicador',
				'tipoarchivo' => 'image',
				'extra' => false
			),
		);
	...
	public function call($accion,$gama=false,$delta=false) {
		if($accion=="lista"){
			...
			if($gama=="indicador"){
				return array(
					"file" => "backend/indicador-nuevo-handler",
					"ajax" => true
				);
			}
	...
	public function getDatas($mid, $tabla, $grafica, $campo="indicador_id", $order="DESC"){
		$stmt = $this->db->prepare("SELECT * FROM ".DB_PREFIX.$this->databaseprefix."_".$tabla." WHERE indicador_id = ? AND grafica_asignada = ? AND status = ? ORDER BY orden ASC, modificado ASC LIMIT 100");
		$stmt->execute(array($mid, $grafica, 'publico'));
		if($rows = $stmt->fetchAll()){
			return $rows;
		}
	}

	public function getObject($id, $campo, $tabla, $grafica){
		if(is_numeric($id)){
			$stmt = $this->db->prepare("SELECT * FROM ".DB_PREFIX.$this->databaseprefix."_".$tabla." WHERE ".$campo." = ? AND grafica_asignada = ? AND status = ? LIMIT 1");
			$stmt->execute(array($id, $grafica, 'publico'));
			if($row = $stmt->Fetch()){
				return $row;
			}
		}
	...
	public function newObject($mid, $tabla, $grafica){
		$stmt = $this->db->prepare("SELECT indicador_id FROM ".DB_PREFIX.$this->databaseprefix."_".$tabla." WHERE indicador_id = ? AND grafica_asignada = ? AND status = ? LIMIT 1");
		$stmt->execute(array($mid, $grafica, 'revision'));
		$stmt->bindColumn('indicador_id', $id);
		if($row = $stmt->Fetch()){
			return $id;
		}else{
			$stmt = $this->db->prepare("INSERT INTO ".DB_PREFIX.$this->databaseprefix."_".$tabla." (indicador_id, grafica_asignada, status, modificado) VALUES (?, ?, ?, ?)");
			$stmt->execute(array($mid, $grafica, 'publico', date("Y-m-d H:i:s")));
			$mid = $this->db->lastInsertId();
			return $mid;
		}
	}

	public function templateElpaso($obj=false, $deleteurl="",$grafica){
		if($obj){


			echo '<div class="anelm" id="elpasoobj-'.$obj['valor_id'].'">';
			echo '<div class="handle"><div class="hicon"><i class="fa fa-sort" aria-hidden="true"></i></div></div>';
			echo '<input type="hidden" name="valor_id_'.$grafica.'[]" value="'.$obj['valor_id'].'">';
			echo '<input type="hidden" name="valor_orden_'.$grafica.'[]" value="">';
			echo '<div class="aeacts"><a href="'.$deleteurl.$obj['valor_id'].'/'.$grafica.'?action=delete-object" class="btn btn-danger btn-xs delete-object-btn">Eliminar</a></div>';
			echo '<div class="row">
				<div class="col-sm-4">
					<div class="form-group">
						<label class="control-label">Nombre</label>
						<input type="text" class="form-control" name="valor_nombre_'.$grafica.'[]" value="'.$obj['valor_nombre'].'">
					</div>
				</div>
				<div class="col-sm-4">
					<div class="form-group">
						<label class="control-label">Valor numérico</label>
						<input type="text" class="form-control" name="valor_val_'.$grafica.'[]" value="'.$obj['valor_val'].'">
					</div>
				</div>
				<div class="col-sm-4">
					<div class="form-group">
						<label class="control-label">Texto a mostrar</label>
						<input type="text" class="form-control" name="valor_texto_'.$grafica.'[]" value="'.$obj['valor_texto'].'">
					</div>
				</div>
			</div>';
			echo '</div>';
