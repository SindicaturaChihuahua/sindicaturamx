...
if(logeado() && $user->exists && $user->hasRole($controlador->permiso)){
	...
	$bodys->set( 'bottomjs', URLTHEME.'secciones/'.$controlador->name.'/js/valor.js' );

	//Para BreadCrumbs
	if(isset($_GET['beta']) && $_GET['beta']=='editar' && isset($_GET['delta']) && $obj = $controlador->getPost($_GET['delta'], $_GET['gama'])){
		$cHeaderData['titulo'] = 'Editar indicador';
		$url->addBreadcrumb(array('nombre' => 'Editar indicador', 'link' => 's/'.$controlador->name.'/editar/'.$obj['tipo'].'/'.$obj['post_id']));
		...
		$cHeaderData['titulo'] = 'Nuevo indicador';
		$url->addBreadcrumb(array('nombre' => 'Nuevo indicador', 'link' => 's/'.$controlador->name.'/nuevo/'.$obj['tipo']));
	}else{
	...
	echo '<div class="box">';
            ...
            <div class="col-sm-9">
                <div class="row">
                    <div class="col-sm-12">
						...
						<div class="form-group">
							<label class="control-label">Título imagen interior</label>
					...
					<div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="fijartime2" id="timepicker2" value="<?=datetimepickerFormatDate($obj['modificado2']);?>">
                        </div>
                    </div>
                </div>
        	</div>
		...
		<div class="row">
            ...
            <div class="col-sm-9">
                <div class="row">
                    <div class="col-sm-12">
						<?php
						$textos = json_decode($obj['textos'], true);
						?>
						<div class="form-group">
							<label class="control-label">Título #1</label>
                            <input type="text" class="form-control" name="titulo1" value="<?=cleanOutput($textos[0]["titulo"]);?>">
						</div>
						<div class="form-group">
							<label class="control-label">Texto #1</label>
                            <input type="text" class="form-control" name="texto1" value="<?=cleanOutput($textos[0]["texto"]);?>">
						</div>
						<div class="form-group">
							<label class="control-label">Título #2</label>
                            <input type="text" class="form-control" name="titulo2" value="<?=cleanOutput($textos[1]["titulo"]);?>">
						</div>
						<div class="form-group">
							<label class="control-label">Texto #2</label>
                            <input type="text" class="form-control" name="texto2" value="<?=cleanOutput($textos[1]["texto"]);?>">
						</div>
                    </div>
                </div>
        	</div>
		...
		<?php
		$graficos = json_decode($obj['graficas'], true);
		$gtipos = array(
			"bar" => "Barras",
		...
		<div class="row">
            ...
            <div class="col-sm-9">
                <div class="row">
                    <div class="col-sm-12">
						<div class="form-group">
                            ...
                            <input type="text" class="form-control" name="gtitulo1" value="<?=cleanOutput($graficos[0]["titulo"]);?>">
						</div>
						<div class="form-group">
							<label class="control-label">Descripción</label>
                            <input type="text" class="form-control" name="gdescripcion1" value="<?=cleanOutput($graficos[0]["descripcion"]);?>">
						</div>
					</div>
				</div>
				<div class="row">
                    <div class="col-sm-5">
						<div class="form-group">
							...
							<select class="form-control" name="gtipo1">
								<?php
								foreach ($gtipos as $k => $v) {
									$selected = '';
									if($graficos[0]["tipo"] == $k){
										$selected = ' selected';
									}
						...
						<div class="form-group">
							<label class="control-label">¿Mostrar datos como texto?</label>
							<select class="form-control" name="gmostrardatos1">
								<?php
								foreach ($gopciones as $k => $v) {
									$selected = '';
									if($graficos[0]["mostrardatos"] == $k){
										$selected = ' selected';
									}
									echo '<option value="'.$k.'"'.$selected.'>'.$v.'</option>';
								}
				...
				<div class="row" id="elpasos-0" data-action="<?=$url->createUrl($controlador->static_urls['valor']);?>/nuevo/<?=$obj['post_id'];?>/0">
					<div class="col-sm-12" style="padding-bottom:40px;">
						<h4>Valores</h4>
						<p>Haz clic en el botón “Añadir valor" para agregar valores a la gráfica.</p>
						<a href="#" class="btn btn-default btn-sm addaction"><span class="fa fa-plus"></span> &nbsp;Añadir valor</a>
					</div>
					<div id="elpasoscon-0" class="col-sm-12 sortable">
						<?php
						$elpasos = $controlador->getDatas($obj['post_id'], 'valores', 0);
						if(!empty($elpasos)){
							$deleteurl=$url->createUrl($controlador->static_urls['valor']).'/eliminar/';
							foreach($elpasos as $r){
								$controlador->templateElpaso($r, $deleteurl,0);
							}
						}
						?>
					</div>
            ...
            <div class="col-sm-9">
                <div class="row">
                    <div class="col-sm-12">
						<div class="form-group">
							<label class="control-label">Título</label>
                            <input type="text" class="form-control" name="gtitulo2" value="<?=cleanOutput($graficos[1]["titulo"]);?>">
						</div>
						<div class="form-group">
							<label class="control-label">Descripción</label>
                            <input type="text" class="form-control" name="gdescripcion2" value="<?=cleanOutput($graficos[1]["descripcion"]);?>">
						</div>
					</div>
				</div>
				<div class="row">
                    <div class="col-sm-5">
						<div class="form-group">
							<label class="control-label">Tipo de gráfica</label>
							<select class="form-control" name="gtipo2">
								<?php
								foreach ($gtipos as $k => $v) {
									$selected = '';
									if($graficos[1]["tipo"] == $k){
										$selected = ' selected';
									}
									echo '<option value="'.$k.'"'.$selected.'>'.$v.'</option>';
								}
						...
						<div class="form-group">
							<label class="control-label">¿Mostrar datos como texto?</label>
							<select class="form-control" name="gmostrardatos2">
								<?php
								foreach ($gopciones as $k => $v) {
									$selected = '';
									if($graficos[1]["mostrardatos"] == $k){
										$selected = ' selected';
									}
									echo '<option value="'.$k.'"'.$selected.'>'.$v.'</option>';
								}
				...
				<div class="row" id="elpasos-1" data-action="<?=$url->createUrl($controlador->static_urls['valor']);?>/nuevo/<?=$obj['post_id'];?>/1">
					<div class="col-sm-12" style="padding-bottom:40px;">
						<h4>Valores</h4>
						<p>Haz clic en el botón “Añadir valor" para agregar valores a la gráfica.</p>
						<a href="#" class="btn btn-default btn-sm addaction"><span class="fa fa-plus"></span> &nbsp;Añadir valor</a>
					</div>
					<div id="elpasoscon-1" class="col-sm-12 sortable">
						<?php
						$elpasos = $controlador->getDatas($obj['post_id'], 'valores', 1);
						if(!empty($elpasos)){
							$deleteurl=$url->createUrl($controlador->static_urls['valor']).'/eliminar/';
							foreach($elpasos as $r){
								$controlador->templateElpaso($r, $deleteurl,1);
							}
						}
						?>
					</div>
        ...
        <div class="row object-image-con">
        	<div class="col-sm-3">
            	<h4>Recursos</h4>
                <p>Selecciona los distintos recursos asignados a este indicador.</p>
            </div>
            <div class="col-sm-9">
		...
		<div class="row">
        	...
        	<div class="col-sm-3">
                ...
                <div class="btn btn-default btn-sm btn-fileuploader"><input id="gfu-galeria-files" class="fileupload" type="file" name="files[]" multiple><span class="fa fa-file-o"></span> &nbsp;Cargar Archivos</div>
            </div>
            <div class="col-sm-9">
		...
		*/ ?>
        ...
        <div class="row">
        	<div class="col-sm-3">
            	<h4>Visibilidad</h4>
                <p>Controla si el indicador estará visible en el sitio.</p>
            </div>
            <div class="col-sm-9">
                <div class="row">
                    <div class="col-sm-4">
        ...
        <div class="row informacion-comprimida" id="motores-de-busqueda-row">
        	<div class="col-sm-3">
                ...
                <p>Establece el título de tu página, la meta descripción y la URL amigable. Esto ayuda a definir como el indicador se muestra en los motores de búsqueda.</p>
            </div>
            <div class="col-sm-9">
				<div class="row">
                    <div class="col-sm-12">
                        <div class="form-group">
                            ...
                            <div class="input-group">
                              <span class="input-group-addon"><?=URL;?>como-vamos/</span>
