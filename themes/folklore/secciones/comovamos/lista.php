...
if(logeado() && $user->exists && $user->hasRole($controlador->permiso)){
	...
	$cHeaderData=array(
		...
		'opciones' => array(
			array(
				...
				'link' => $controlador->static_urls['nuevo'].'/indicador',
				'icono' => 'plus'
			),
		)
	);
	...
	$proSQL="SELECT t1.post_id, t1.titulo, t1.tipo, t1.visitas, t1.visibilidad, t1.modificado, t1.orden, t2.pseudonimo FROM ".DB_PREFIX.$controlador->databaseprefix." as t1 JOIN ".DB_PREFIX."users as t2 ON t1.autor_id = t2.uid WHERE t1.status = 'publicado'";
