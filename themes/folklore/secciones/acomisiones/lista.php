...
if(logeado() && $user->exists && $user->hasRole($controlador->permiso)){
	...
	$cHeaderData=array(
		...
		'opciones' => array(
			array(
				'nombre' => 'Sesiones',
				'link' => $controlador->static_urls['sesiones'],
				'icono' => 'users'
			),
			array(
				'nombre' => 'Agregar Comisión',
				'link' => $controlador->static_urls['nuevo'].'/comision',
				'icono' => 'plus'
			),
	...
	$proSQL="SELECT t1.post_id, t1.titulo, t1.tipo, t1.visitas, t1.visibilidad, t1.orden, t1.modificado, t2.pseudonimo FROM ".DB_PREFIX.$controlador->databaseprefix." as t1 JOIN ".DB_PREFIX."users as t2 ON t1.autor_id = t2.uid WHERE t1.status = 'publicado'";
