<?php
class back_acomisiones extends Seccion {

	function __construct($db)
	{
		$this->db = $db;
		$this->backclass = "back_acomisiones";
		$this->frontclass = "front_acomisiones";
        $this->name = "acomisiones";
        $this->order = '1';
        $this->version = '1.1.0';
		$this->tab = 'editor';
		...
		$this->static_urls = array(
			...
			'orden-handler' => 's/'.$this->name.'/orden-handler',
			'sesiones' => 's/'.$this->name.'/sesiones',
			'sesiones-nuevo' => 's/'.$this->name.'/sesiones/nuevo',
			'sesiones-editar' => 's/'.$this->name.'/sesiones/editar',
			'sesiones-nuevo-handler' => 's/'.$this->name.'/sesiones/sesiones-nuevo-handler',
			'sesiones-single-upload-handler' => 's/'.$this->name.'/sesiones/sesiones-single-upload-handler',
		);
		...
		$this->single_files = array(
			'cover' => array(
				...
				'info' => 'Selecciona la imagen principal para identificar este comisión',
				'tipoarchivo' => 'image',
				'extra' => false
			),
			'archivo' => array(
				...
				'info' => 'Selecciona un pdf descargable',
				'tipoarchivo' => 'file',
				'extra' => false
			)
		);
		...
		$this->sesiones_single_files = array(
			'sesion_acta' => array(
				'required' => array("pdf","doc","docx","jpg","jpge","jpeg","gif","png"),
				'maxfilesize' => '10',
	...
	public function call($accion,$gama=false,$delta=false) {
		if($accion=="lista"){
			...
			if($gama=="comision"){
				return array(
					"file" => "backend/comision-nuevo-handler",
					"ajax" => true
				);
			}
