<?php
class back_transparencia extends Seccion {

	function __construct($db)
	{
		$this->db = $db;
		$this->backclass = "back_transparencia";
		$this->frontclass = "front_transparencia";
        $this->name = "transparencia";
        $this->order = '1';
        $this->version = '1.1.0';
		$this->tab = 'editor';
	...
	public function call($accion,$gama=false,$delta=false) {
		if($accion=="lista"){
			...
			if($gama=="informacion"){
				return array(
					"file" => "backend/informacion-nuevo-handler",
					"ajax" => true
				);
			}
