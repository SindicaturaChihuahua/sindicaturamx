<?php
class back_noticias extends Seccion {

	function __construct($db)
	{
		$this->db = $db;
		$this->backclass = "back_noticias";
		$this->frontclass = "front_noticias";
        $this->name = "noticias";
        $this->order = '1';
        $this->version = '1.1.0';
		$this->tab = 'editor';
		...
		$this->single_files = array(
			'cover' => array(
				...
				'info' => 'Selecciona la imagen principal del nota',
				'tipoarchivo' => 'image',
				'extra' => false
			)
		);
	...
	public function call($accion,$gama=false,$delta=false) {
		if($accion=="lista"){
			...
			if($gama=="nota"){
				return array(
					"file" => "backend/nota-nuevo-handler",
					"ajax" => true
				);
			}
