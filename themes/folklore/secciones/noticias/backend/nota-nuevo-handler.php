		...
		if($obj && $obj['tipo']==$tipo){
			...
			if($obj['autor_id'] == $user->uid || $user->hasRole("EditorEnJefe")){
				if($accion=="editar"){
					...
					$updateFields=array('autor_id', 'titulo', 'video', 'descripcion', 'destacado', 'slug', 'seo_titulo', 'seo_descripcion', 'visibilidad', 'creado', 'modificado', 'status', 'extra');
					...
					if(count($_POST['s_categorias'])<1){
						$errores[]="Debes seleccionar una categoría para la nota";
					}
