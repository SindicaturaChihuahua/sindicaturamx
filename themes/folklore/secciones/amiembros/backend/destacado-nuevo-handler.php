		...
		if($obj && $obj['tipo']==$tipo){
			...
			if($obj['autor_id'] == $user->uid || $user->hasRole("EditorEnJefe")){
				if($accion=="editar"){
					...
					$updateFields=array('autor_id', 'titulo', 'tagline', 'descripcion', 'enlaces', 'slug', 'seo_titulo', 'seo_descripcion', 'visibilidad', 'creado', 'modificado', 'status', 'extra');
					...
					if(empty($errores)){
						$orden = 0;
						$elpasosFields=array(
							'orden', 'comision_asignada', 'comision_puesto',
							'status', 'modificado'
						);

						if(isset($_POST['comision_id']) && count($_POST['comision_id']>0)){
							$defaults=array();
							foreach($_POST['comision_id'] as $key => $mid){
								if(is_numeric($mid)){
									$elpasosData=array();
									foreach($defaults as $d){
										$elpasosData[$d]=$_POST['comision_'.$d][$key];
									}
									$elpasosData['comision_asignada']=$_POST['comision_asignada'][$key];
									$elpasosData['comision_puesto']=$_POST['comision_puesto'][$key];
									$elpasosData['status']='publico';

									$elpasosData['orden']=$orden;
									if(!empty($elpasosData) && empty($errores)){
										$stmt = $db->prepare("UPDATE ".DB_PREFIX.$controlador->databaseprefix."_comisiones SET ".pdoSet($elpasosFields, $elpasosData, $elpasosData)." WHERE comision_id = :theobjid");
