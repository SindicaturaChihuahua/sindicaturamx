...
if(logeado() && $user->exists && $user->hasRole($controlador->permiso)){
	...
	if(isset($_GET['beta']) && $_GET['beta']=='editar' && isset($_GET['gama']) && $blog_post = $controlador->getPost($_GET['gama'])){
		$cHeaderData['titulo'] = 'Editar aliado';
		$url->addBreadcrumb(array('nombre' => 'Editar aliado', 'link' => 's/'.$controlador->name.'/editar/'.$blog_post['post_id']));
	}else if(isset($_GET['beta']) && $_GET['beta']=='nuevo' && $post_id = $controlador->newPost($user->uid)){
		$blog_post = $controlador->getPost($post_id);
		$cHeaderData['titulo'] = 'Nuevo aliado';
		$url->addBreadcrumb(array('nombre' => 'Nuevo aliado', 'link' => 's/'.$controlador->name.'/nuevo'));
	}else{
	...
	echo '<div class="box">';
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="form-group">
                            <label class="control-label">Nombre (control interno)</label>
        ...
        <div class="row object-image-con">
        	<div class="col-sm-3">
            	<h4>Recursos</h4>
                <p>Selecciona los distintos recursos que tu entrada utilizara.</p>
            </div>
            <div class="col-sm-9">
        ...
        <div class="row">
        	<div class="col-sm-3">
            	<h4>Visibilidad</h4>
                <p>Controla si esta entrada puede ser vista en tu sitio web.</p>
            </div>
            <div class="col-sm-9">
                <div class="row">
                    <div class="col-sm-5">
