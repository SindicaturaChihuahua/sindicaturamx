...
if(logeado() && $user->exists && $user->hasRole($controlador->permiso)){
	...
	$cHeaderData=array(
		...
		'opciones' => array(
			array(
				'nombre' => 'Agregar Página',
				'link' => $controlador->static_urls['nuevo'].'/landing',
				'icono' => 'plus'
			),
			array(
				'nombre' => 'Agregar Recurso',
				'link' => $controlador->static_urls['nuevo'].'/recurso',
				'icono' => 'plus'
			),
		)
	);
	...
	$proSQL="SELECT t1.post_id, t1.titulo, t1.tipo, t1.visitas, t1.visibilidad, t1.modificado, t1.slug, t2.pseudonimo FROM ".DB_PREFIX.$controlador->databaseprefix." as t1 JOIN ".DB_PREFIX."users as t2 ON t1.autor_id = t2.uid WHERE t1.status = 'publicado'";
	...
	echo '<div class="boxcon"><div class="box">';
		...
		<table class="table table-striped">
            ...
            <tr>
				...
				<td>
					<a href="<?=URL;?>informacion/<?=$o['slug'];?>" target="_blank">Ver <i class="fa fa-external-link" aria-hidden="true"></i></a>
				</td>
            </tr>
            <?php
			}
