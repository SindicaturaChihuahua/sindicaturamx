...
if(logeado() && $user->exists && $user->hasRole($controlador->permiso)){
	...
	if(isset($_GET['beta']) && $_GET['beta']=='editar' && isset($_GET['delta']) && $obj = $controlador->getPost($_GET['delta'], $_GET['gama'])){
		$cHeaderData['titulo'] = 'Editar página';
		$url->addBreadcrumb(array('nombre' => 'Editar página', 'link' => 's/'.$controlador->name.'/editar/'.$obj['tipo'].'/'.$obj['post_id']));
		...
		$cHeaderData['titulo'] = 'Nueva página';
		$url->addBreadcrumb(array('nombre' => 'Nueva página', 'link' => 's/'.$controlador->name.'/nuevo/'.$obj['tipo']));
	}else{
	...
	echo '<div class="box">';
		...
		<div class="row">
        	<div class="col-sm-3">
            	<h4>Información</h4>
				<?php
				$bloque1 = json_decode($obj["bloque1"],true);
				?>
            </div>
            <div class="col-sm-9">
                <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                            <label class="control-label">Título</label>
                            <input type="text" class="form-control" name="bloque1_titulo" value="<?=cleanOutput($bloque1["titulo"]);?>">
                        </div>
					</div>
					<div class="col-sm-12">
						<div class="form-group">
                            <label class="control-label">Texto</label>
                            <textarea name="bloque1_texto" class="aneditor" autocomplete="off"><?=$bloque1["texto"];?></textarea>
                        </div>
                    </div>
                </div>
        	</div>
		...
		<div class="row">
        	<div class="col-sm-3">
            	<h4>Textos botones</h4>
            </div>
            <div class="col-sm-9">
                <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                            <label class="control-label">Texto Botón #1</label>
                            <input type="text" class="form-control" name="btntxt1" value="<?=cleanOutput($obj["btntxt1"]);?>">
                        </div>
					</div>
                </div>
				<div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                            <label class="control-label">Texto Botón #2</label>
                            <input type="text" class="form-control" name="btntxt2" value="<?=cleanOutput($obj["btntxt2"]);?>">
                        </div>
					</div>
                </div>
				<div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                            <label class="control-label">Texto Botón #3</label>
                            <input type="text" class="form-control" name="btntxt3" value="<?=cleanOutput($obj["btntxt3"]);?>">
                        </div>
					</div>
                </div>
        	</div>
		...
		*/
        ...
        <div class="row">
        	<div class="col-sm-3">
            	<h4>Visibilidad</h4>
                <p>Controla si la página estará disponible públicamente.</p>
            </div>
            <div class="col-sm-9">
                <div class="row">
                    <div class="col-sm-4">
