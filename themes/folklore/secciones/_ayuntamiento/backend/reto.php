...
if(logeado() && $user->exists && $user->hasRole($controlador->permiso)){
	...
	$bodys->set( 'bottomjs', URLTHEME.'secciones/'.$controlador->name.'/js/beneficio.js' );

	//Para BreadCrumbs
	if(isset($_GET['beta']) && $_GET['beta']=='editar' && isset($_GET['delta']) && $obj = $controlador->getPost($_GET['delta'], $_GET['gama'])){
		$cHeaderData['titulo'] = 'Editar reto';
		$url->addBreadcrumb(array('nombre' => 'Editar reto', 'link' => 's/'.$controlador->name.'/editar/'.$obj['tipo'].'/'.$obj['post_id']));
		...
		$cHeaderData['titulo'] = 'Nuevo reto';
		$url->addBreadcrumb(array('nombre' => 'Nuevo reto', 'link' => 's/'.$controlador->name.'/nuevo/'.$obj['tipo']));
	}else{
	...
	echo '<div class="box">';
                ...
                <div class="row">
                    <div class="col-sm-12">
						...
						<div class="form-group">
                            ...
                            <input type="text" class="form-control" name="video" value="<?=cleanOutput($obj['video']);?>">
                        </div>
					</div>
					<div class="col-sm-4">
						<div class="form-group">
							...
							<div class="input-group">
								...
								<input type="text" class="form-control" name="precio" value="<?=cleanOutput($obj['precio']);?>">
					...
					<div class="col-sm-12">
						...
						<div class="form-group">
                            ...
                            <textarea name="objetivos" class="aneditor" autocomplete="off"><?=$obj['objetivos'];?></textarea>
                        </div>
                        <div class="form-group">
						...
						<div class="form-group">
                            ...
                            <input type="text" class="form-control" name="fijartime2" id="timepicker2" value="<?=datetimepickerFormatDate($obj['fin']);?>">
                        </div>
                    </div>
                </div>
        	</div>
        ...
        <hr>
		<!-- Elpasos -->
		<div class="row" id="elpasos" data-action="<?=$url->createUrl($controlador->static_urls['beneficio']);?>/nuevo/<?=$obj['post_id'];?>">
			<div class="col-sm-3">
				<h4>Beneficios</h4>
				<p>Haz clic en el botón “Añadir beneficio" para agregar más beneficios.</p>
				<a href="#" class="btn btn-default btn-sm addaction"><span class="fa fa-plus"></span> &nbsp;Añadir beneficio</a>
			</div>
			<div id="elpasoscon" class="col-sm-9 sortable">
				<?php
				$elpasos = $controlador->getDatas($obj['post_id'], 'beneficios');
				if(!empty($elpasos)){
        ...
        <div class="row object-image-con">
        	<div class="col-sm-3">
            	<h4>Recursos</h4>
                <p>Selecciona los distintos recursos asignados a esta nota.</p>
            </div>
            <div class="col-sm-9">
		...
		*/ ?>
        ...
        <div class="row">
        	<div class="col-sm-3">
            	<h4>Visibilidad</h4>
                <p>Controla si el servicio estará visible en el sitio.</p>
            </div>
            <div class="col-sm-9">
                <div class="row">
                    <div class="col-sm-4">
        ...
        <div class="row informacion-comprimida" id="motores-de-busqueda-row">
        	<div class="col-sm-3">
                ...
                <p>Establece el título de tu página, la meta descripción y la URL amigable. Esto ayuda a definir como la producto se muestra en los motores de búsqueda.</p>
            </div>
            <div class="col-sm-9">
				<div class="row">
                    <div class="col-sm-12">
                        <div class="form-group">
                            ...
                            <div class="input-group">
                              <span class="input-group-addon"><?=URL;?>retos/</span>
