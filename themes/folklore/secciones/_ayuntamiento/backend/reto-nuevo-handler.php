		...
		if($obj && $obj['tipo']==$tipo){
			...
			if($obj['autor_id'] == $user->uid || $user->hasRole("EditorEnJefe")){
				if($accion=="editar"){
					...
					$updateFields=array('autor_id', 'titulo', 'descripcion', 'objetivos', 'precio', 'video', 'slug', 'seo_titulo', 'seo_descripcion', 'visibilidad', 'creado', 'modificado', 'fin', 'status', 'extra');
					...
					if(!isset($_POST['titulo']) || !validaGeneral($_POST['titulo'],3,280)){
						...
						$updateData['objetivos'] = $_POST['objetivos'];
						$updateData['precio'] = $_POST['precio'];
						$updateData['video'] = $_POST['video'];
					...
					if(empty($errores)){
						$orden = 0;
						$elpasosFields=array(
							'orden', 'beneficio_nombre', 'beneficio_icono',
							'status', 'modificado'
						);

						if(isset($_POST['beneficio_id']) && count($_POST['beneficio_id']>0)){
							$defaults=array();
							foreach($_POST['beneficio_id'] as $key => $mid){
								if(is_numeric($mid)){
									$elpasosData=array();
									foreach($defaults as $d){
										$elpasosData[$d]=$_POST['beneficio_'.$d][$key];
									}

									if( !isset($_POST['beneficio_nombre'][$key]) || !validaGeneral($_POST['beneficio_nombre'][$key],1) ){
										$errores[]="Ingresa el nombre para el beneficio #".($key+1);
									}else{
										$elpasosData['beneficio_nombre']=$_POST['beneficio_nombre'][$key];
									}
									$elpasosData['beneficio_icono']=$_POST['beneficio_icono'][$key];
									$elpasosData['status']='publico';

									$elpasosData['orden']=$orden;
									if(!empty($elpasosData) && empty($errores)){
										$stmt = $db->prepare("UPDATE ".DB_PREFIX.$controlador->databaseprefix."_beneficios SET ".pdoSet($elpasosFields, $elpasosData, $elpasosData)." WHERE beneficio_id = :theobjid");
					...
					if(empty($errores)){
						if($obj['status']=='revision'){
							...
							$updateData['fin'] = getFechaActualSinDiferencia();
						...
						if(isset($_POST['fijartime2']) && datetimeReal($_POST['fijartime2'])){
							if($obj['fin']!=$_POST['fijartime2']){
								$updateData['fin'] = addDiffFormatDate($_POST['fijartime2']);
							}
						}else{
							if($obj['status']!='revision'){
								$errores[]="Ingresa la fecha en formato válido";
