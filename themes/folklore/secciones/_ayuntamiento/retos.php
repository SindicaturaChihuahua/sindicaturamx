<?php
class back_retos extends Seccion {

	function __construct($db)
	{
		$this->db = $db;
		$this->backclass = "back_retos";
		$this->frontclass = "front_retos";
        $this->name = "retos";
        $this->order = '1';
        $this->version = '1.1.0';
		$this->tab = 'editor';
		$this->displayName = 'Soluciones';
		$this->description = 'Descripcion';
		$this->icon = 'edit';
		$this->databaseprefix = 's_retos';
		$this->permiso = 'Editor';
		$this->static_urls = array(
			...
			'beneficio' => 's/'.$this->name.'/beneficio',
		...
		$this->single_files = array(
			'cover' => array(
				...
				'info' => 'Selecciona la imagen principal del reto',
				'tipoarchivo' => 'image',
				'extra' => false
			)
		);
	...
	public function call($accion,$gama=false,$delta=false) {
		if($accion=="lista"){
			...
			if($gama=="reto"){
				return array(
					"file" => "backend/reto-nuevo-handler",
					"ajax" => true
				);
			}
		}else if($accion=="beneficio"){
	...
	public function getDatas($mid, $tabla, $campo="reto_id", $order="DESC"){
		$stmt = $this->db->prepare("SELECT t1.* FROM ".DB_PREFIX.$this->databaseprefix."_".$tabla." AS t1 WHERE t1.reto_id = ? AND t1.status = ? ORDER BY t1.orden ASC, t1.modificado ASC LIMIT 100");
	...
	public function newObject($mid, $tabla){
		$stmt = $this->db->prepare("SELECT reto_id FROM ".DB_PREFIX.$this->databaseprefix."_".$tabla." WHERE reto_id = ? AND status = ? LIMIT 1");
		$stmt->execute(array($mid, 'revision'));
		$stmt->bindColumn('reto_id', $id);
		if($row = $stmt->Fetch()){
			return $id;
		}else{
			$stmt = $this->db->prepare("INSERT INTO ".DB_PREFIX.$this->databaseprefix."_".$tabla." (reto_id, status, modificado) VALUES (?, ?, ?)");
	...
	public function templateElpaso($obj=false, $deleteurl=""){
		if($obj){
			$options = '';
			$iconos = array(
				"video" => "Video",
			...
			foreach($iconos as $icono => $texto){
				if($obj['beneficio_icono'] == $icono){
					$options .= '<option value="'.$icono.'" selected>'.$texto.'</option>';
				}else{
					$options .= '<option value="'.$icono.'">'.$texto.'</option>';
				}
			}

			echo '<div class="anelm" id="elpasoobj-'.$obj['beneficio_id'].'">';
			echo '<div class="handle"><div class="hicon"><i class="fa fa-sort" aria-hidden="true"></i></div></div>';
			echo '<input type="hidden" name="beneficio_id[]" value="'.$obj['beneficio_id'].'">';
			echo '<input type="hidden" name="beneficio_orden[]" value="">';
			echo '<div class="aeacts"><a href="'.$deleteurl.$obj['beneficio_id'].'?action=delete-object" class="btn btn-danger btn-xs delete-object-btn">Eliminar</a></div>';
			echo '<div class="row">
				<div class="col-sm-4">
					<div class="form-group">
						<label class="control-label">Nombre</label>
						<input type="text" class="form-control" name="beneficio_nombre[]" value="'.$obj['beneficio_nombre'].'">
					</div>
					<div class="form-group">
						<label class="control-label">Ícono</label>
						<select class="form-control" name="beneficio_icono[]">
							'.$options.'
						</select>
					</div>
				</div>
