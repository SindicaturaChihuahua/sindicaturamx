...
if(logeado() && $user->exists && $user->hasRole($controlador->permiso)){
	...
	if(isset($_GET['beta']) && $_GET['beta']=='editar' && isset($_GET['gama']) && $blog_post = $controlador->getPost($_GET['gama'])){
		$cHeaderData['titulo'] = 'Plan de Desarrollo';
		// $url->addBreadcrumb(array('nombre' => 'Editar', 'link' => 's/'.$controlador->name.'/editar/'.$blog_post['post_id']));
	}else if(isset($_GET['beta']) && $_GET['beta']=='nuevo' && $post_id = $controlador->newPost($user->uid)){
		$blog_post = $controlador->getPost($post_id);
		$cHeaderData['titulo'] = 'Nuevo';
		$url->addBreadcrumb(array('nombre' => 'Nuevo', 'link' => 's/'.$controlador->name.'/nuevo'));
	}else{
	...
	echo '<div class="box">';
                ...
                <div class="row">
                    <div class="col-sm-12">
						...
						<div class="form-group">
                            ...
                            <textarea name="descripcion" class="aneditor" autocomplete="off"><?=$blog_post['descripcion'];?></textarea>
                        </div>
						<div class="row">
							<div class="form-group col-sm-3">
								...
								<select class="form-control" name="formato">
									...
									foreach ($formatos as $formato => $texto) {
										$selected = '';
										if($blog_post['formato'] == $formato){
											$selected = ' selected';
										}
										echo '<option value="'.$formato.'"'.$selected.'>'.$texto.'</option>';
									}
									?>
								</select>
							</div>
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="btn_texto" value="<?=cleanOutput($blog_post['btn_texto']);?>">
                        </div>
                        <div class="form-group">
		...
		*/ ?>
        ...
        <div class="row">
        	<div class="col-sm-3">
            	<h4>Visibilidad</h4>
                <p>Controla si este archivo puede ser visto en tu sitio web.</p>
            </div>
            <div class="col-sm-9">
                <div class="row">
                    <div class="col-sm-5">
