		...
		if($obj && $obj['tipo']==$tipo){
			...
			if($obj['autor_id'] == $user->uid || $user->hasRole("EditorEnJefe")){
				if($accion=="editar"){
					...
					$updateFields=array('autor_id', 'categoria_id', 'titulo', 'descripcion','sesion_lugar','slug', 'seo_titulo', 'seo_descripcion', 'visibilidad', 'creado', 'modificado', 'status', 'extra');

					if(!isset($_POST['titulo']) || !validaGeneral($_POST['titulo'],3,280)){
						$errores[]="Ingresa un título válido";
					}else{
						$updateData['categoria_id'] = $_POST["categoria_id"];
						$updateData['titulo'] = $_POST['titulo'];
						$updateData['descripcion'] = $_POST['descripcion'];
						$updateData['sesion_lugar'] = $_POST['sesion_lugar'];
						// if(isset($_POST["destacado"]) && !empty($_POST["destacado"])){
						// 	$updateData["destacado"] = 1;
						// }else{
						// 	$updateData["destacado"] = 0;
						// }
					...
					if(!empty($updateData) && empty($errores)){
						...
						if($stmt->execute($updateData)){
							...
							if($obj["correoenviado"]==0){
								if(isset($_POST['correoenviado']) && $_POST['correoenviado']==1){
									// Enviar correo
									...
									$campana = $MailChimp->post("campaigns", [
										'type' => "regular",
										'recipients' => array(
											'list_id' => '7f4fd6e1e9'
										),
										'settings' => array(
											'subject_line' => $updateData['titulo']." - Calendario de regidores",
											'title' => $updateData['titulo']." - Calendario de regidores",
											'reply_to' => 'comunicacionsindicatura18.21@gmail.com',
											'from_name' => 'Sindicatura Chihuahua'
										)
									]);
									if($MailChimp->success()){

										$htmlmensaje = "<h1>Calendario de regidores</h1>";
										$htmlmensaje .= "<p>Evento: <strong>".$updateData['titulo']."</strong></p>";
										$htmlmensaje .= "<p>Lugar: <strong>".$updateData['sesion_lugar']."</strong></p>";
										$htmlmensaje .= "<p>Dia y hora: <strong>".formatDate($updateData['modificado'], "siga")."</strong></p>";

										$template = $MailChimp->put("campaigns/".$campana['id']."/content", [
											'html' => $htmlmensaje
										]);
										if($MailChimp->success()){
											$MailChimp->post("campaigns/".$campana['id']."/actions/send");
											$reload .= "?mc_campana=".$campana['id'];
											$stmt = $db->prepare("UPDATE ".DB_PREFIX.$controlador->databaseprefix." SET correoenviado = ? WHERE post_id = ?");
											if($stmt->execute(array(1, $obj['post_id']))){
												$reload .= "&correoenviado=1";
											}
										}
									}
								}
