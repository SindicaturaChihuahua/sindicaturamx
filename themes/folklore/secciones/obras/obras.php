<?php
class back_obras extends Seccion {

	function __construct($db)
	{
		$this->db = $db;
		$this->backclass = "back_obras";
		$this->frontclass = "front_obras";
        $this->name = "obras";
        $this->order = '1';
        $this->version = '1.1.0';
		$this->tab = 'editor';
		...
		$this->single_files = array(
			'archivo' => array(
				'required' => array("kml","xml","jpg","jpge","jpeg","gif","png"),
				'maxfilesize' => '10',
				'info_btn' => '<span class="fa fa-file-image-o"></span> &nbsp;Archivo KML',
				'info' => 'Selecciona el archivo en formato .kml',
				'tipoarchivo' => 'file',
				'extra' => false
			),
		);
		...
		$this->years = getYears();
		$this->colores = getColores();
		$this->secciones = getCategoriasObras();
	}
	...
	public function call($accion,$gama=false,$delta=false) {
		if($accion=="lista"){
			...
			if($gama=="obra"){
				return array(
					"file" => "backend/obra-nuevo-handler",
					"ajax" => true
				);
			}
	...
	public function getDatas($mid, $tabla, $grafica, $campo="obra_id", $order="DESC"){
		$stmt = $this->db->prepare("SELECT * FROM ".DB_PREFIX.$this->databaseprefix."_".$tabla." WHERE obra_id = ? AND grafica_asignada = ? AND status = ? ORDER BY orden ASC, modificado ASC LIMIT 100");
	...
	public function newObject($mid, $tabla, $grafica){
		$stmt = $this->db->prepare("SELECT obra_id FROM ".DB_PREFIX.$this->databaseprefix."_".$tabla." WHERE obra_id = ? AND grafica_asignada = ? AND status = ? LIMIT 1");
		$stmt->execute(array($mid, $grafica, 'revision'));
		$stmt->bindColumn('obra_id', $id);
		if($row = $stmt->Fetch()){
			return $id;
		}else{
			$stmt = $this->db->prepare("INSERT INTO ".DB_PREFIX.$this->databaseprefix."_".$tabla." (obra_id, grafica_asignada, status, modificado) VALUES (?, ?, ?, ?)");
