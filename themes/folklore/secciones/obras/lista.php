...
if(logeado() && $user->exists && $user->hasRole($controlador->permiso)){
	...
	$cHeaderData=array(
		...
		'opciones' => array(
			array(
				'nombre' => 'Agregar Obra',
				'link' => $controlador->static_urls['nuevo'].'/obra',
				'icono' => 'plus'
			),
		)
	);
	...
	$proSQL="SELECT t1.post_id, t1.titulo, t1.tipo, t1.categoria, t1.seccion, t1.visitas, t1.visibilidad, t1.modificado, t1.orden, t2.pseudonimo FROM ".DB_PREFIX.$controlador->databaseprefix." as t1 JOIN ".DB_PREFIX."users as t2 ON t1.autor_id = t2.uid WHERE t1.status = 'publicado'";
	...
	echo '<div class="boxcon"><div class="box">';
		...
		<table class="table table-striped">
            ...
            <tr id="obj_<?=$o['post_id'];?>">
                ...
                <td><?=$controlador->years[$o["categoria"]];?></td>
				<td><?=$controlador->secciones[$o["seccion"]];?></td>
