...
if(logeado() && $user->exists && $user->hasRole($controlador->permiso)){
	...
	if(isset($_GET['beta']) && $_GET['beta']=='editar' && isset($_GET['delta']) && $obj = $controlador->getPost($_GET['delta'], $_GET['gama'])){
		$cHeaderData['titulo'] = 'Editar obra';
		$url->addBreadcrumb(array('nombre' => 'Editar obra', 'link' => 's/'.$controlador->name.'/editar/'.$obj['tipo'].'/'.$obj['post_id']));
		...
		$cHeaderData['titulo'] = 'Nueva obra';
		$url->addBreadcrumb(array('nombre' => 'Nueva obra', 'link' => 's/'.$controlador->name.'/nuevo/'.$obj['tipo']));
	}else{
	...
	echo '<div class="box">';
            ...
            <div class="col-sm-9">
					...
					<div class="col-sm-3">
                        <div class="form-group">
							...
							<select class="form-control" name="categoria">
								<?php
								foreach($controlador->years as $year) {
									$selected = '';
									if(empty($obj["categoria"]) && $year == date("Y")){
										$selected = ' selected';
									}elseif(!empty($obj["categoria"]) && $year == $obj["categoria"]){
										$selected = ' selected';
									}
						...
						<div class="form-group">
							...
							<select class="form-control" name="seccion">
								<?php
								foreach($controlador->secciones as $val => $seccion) {
									$selected = ($val == $obj["seccion"]) ? ' selected' : '' ;
									echo '<option value="'.$val.'"'.$selected.'>'.$seccion.'</option>';
								}
								?>
							</select>
                        </div>
				...
				<div class="row">
					<div class="col-sm-12">
						...
						<div class="row">
						<?php
						for ($i=0; $i < count($controlador->colores); $i++) {
							$checked = '';
							if($controlador->colores[$i] == $obj["color"]){
								$checked = ' checked';
							}
							// echo '<div class="col-sm-3" style="text-align:center; padding-bottom:15px;">
							// 	<label style="cursor:pointer; display:block; width:100%; padding-top:16%; padding-bottom:16%; background-color:'.$controlador->colores[$i].';"><input type="radio" value="'.$controlador->colores[$i].'" name="color"'.$checked.'></label>
							// </div>';
							echo '<div class="col-sm-1" style="text-align:center; padding-bottom:30px;">
								<label style="position:relative; cursor:pointer; display:block; width:100%; padding-top:100%; border-radius:50%; background-color:'.$controlador->colores[$i].';"><input type="radio" value="'.$controlador->colores[$i].'" name="color"'.$checked.' style="position:absolute; bottom:0; left:50%; transform:translateY(130%) translateX(-50%); margin-top:10px; cursor:pointer;"></label>
							</div>';
						}
						?>
						</div>
        ...
        <div class="row object-image-con">
        	<div class="col-sm-3">
            	<h4>Recursos</h4>
                <p>Selecciona los distintos recursos asignados a esta obra.</p>
            </div>
            <div class="col-sm-9">
                    ...
                    <div class="col-sm-12">
                        <div class="form-group">
                            ...
                            <div class="input-group">
                              <span class="input-group-addon"><?=URL;?>obras/</span>
