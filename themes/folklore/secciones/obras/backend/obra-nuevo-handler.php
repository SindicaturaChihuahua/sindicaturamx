		...
		if($obj && $obj['tipo']==$tipo){
			...
			if($obj['autor_id'] == $user->uid || $user->hasRole("EditorEnJefe")){
				if($accion=="editar"){
					...
					$updateFields=array('autor_id', 'titulo', 'categoria', 'seccion', 'color', 'slug', 'seo_titulo', 'seo_descripcion', 'visibilidad', 'creado', 'modificado', 'status', 'extra');
					...
					if(!isset($_POST['titulo']) || !validaGeneral($_POST['titulo'],3,280)){
						...
						$updateData['categoria'] = $_POST['categoria'];
						$updateData['seccion'] = $_POST['seccion'];
