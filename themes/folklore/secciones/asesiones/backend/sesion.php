...
if(logeado() && $user->exists && $user->hasRole($controlador->permiso)){
	...
	if(isset($_GET['beta']) && $_GET['beta']=='editar' && isset($_GET['delta']) && $obj = $controlador->getPost($_GET['delta'], $_GET['gama'])){
		...
		$url->addBreadcrumb(array('nombre' => 'Editar sesión', 'link' => 's/'.$controlador->name.'/editar/'.$obj['tipo'].'/'.$obj['post_id']));
		...
		$url->addBreadcrumb(array('nombre' => 'Nueva sesión', 'link' => 's/'.$controlador->name.'/nuevo/'.$obj['tipo']));
	}else{
		...
		*/ ?>
        ...
        <div class="row informacion-comprimida" id="motores-de-busqueda-row">
        	<div class="col-sm-3">
                ...
                <p>Establece el título de tu página, la meta descripción y la URL amigable. Esto ayuda a definir como la sesión se muestra en los motores de búsqueda.</p>
            </div>
            <div class="col-sm-9">
				<div class="row">
                    <div class="col-sm-12">
                        <div class="form-group">
                            ...
                            <div class="input-group">
                              <span class="input-group-addon"><?=URL;?>ayuntamiento/sesiones-de-cabildo/</span>
